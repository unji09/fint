...
class ActivityRepositoryFindDetailTest {
    ...
    void findsActivityWhenOwnerBelongsToCurrentTenant() {
        ...
        authenticateAs(tenant.getTenantId());

        Optional<Activity> result = activityRepository.findDetail(activity.getActivityId());
    ...
    void rejectsActivityOfAnotherTenant() {
        ...
        authenticateAs(tenantB.getTenantId());
    ...
    private void authenticateAs(Long tenantId) {
        CustomUserDetails principal = new CustomUserDetails(1L, tenantId, "MEMBER");
