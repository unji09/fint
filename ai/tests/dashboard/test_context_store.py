...
class TestContextStore:
    async def test_empty_context_returns_empty_list(self, store):
        result = await store.get_context(dashboard_id=1, user_id=1)
        assert result == []
    ...
    async def test_different_users_isolated(self, store):
        await store.add_entry(dashboard_id=1, user_id=1, input_text="유저1 질의", search_type="STRUCTURED")
        await store.add_entry(dashboard_id=1, user_id=2, input_text="유저2 질의", search_type="SEMANTIC")

        user1 = await store.get_context(dashboard_id=1, user_id=1)
        user2 = await store.get_context(dashboard_id=1, user_id=2)
    ...
    async def test_different_dashboards_isolated(self, store):
        await store.add_entry(dashboard_id=1, user_id=1, input_text="대시보드1", search_type="STRUCTURED")
        await store.add_entry(dashboard_id=2, user_id=1, input_text="대시보드2", search_type="STRUCTURED")

        d1 = await store.get_context(dashboard_id=1, user_id=1)
        d2 = await store.get_context(dashboard_id=2, user_id=1)
    ...
    async def test_ttl_set_on_write(self, store, fake_redis):
        await store.add_entry(dashboard_id=1, user_id=1, input_text="테스트", search_type="STRUCTURED")

        key = "dashboard:context:1:1"
        assert key in fake_redis._ttls
        assert fake_redis._ttls[key] == TTL_SECONDS

    async def test_clear_context(self, store):
        await store.add_entry(dashboard_id=1, user_id=1, input_text="테스트", search_type="STRUCTURED")
        await store.clear_context(dashboard_id=1, user_id=1)
